...
if [[ ! -d "$HOME/.oh-my-zsh" ]]; then
  ...
  # KEEP_ZSHRC=yes is passed but CANNOT be relied on. Read the installer:
  # setup_zshrc() only consults it *inside* `if [ -f ~/.zshrc ] || [ -h ~/.zshrc ]`.
  # On a fresh box there is no ~/.zshrc, so that branch never runs and the
  # template is written unconditionally — and a fresh box is the only case this
  # script exists for. The flag is kept for the re-run case; the cleanup below is
  # what actually protects the config.
...
# Generated-config cleanup. Anything that writes a dotfile we also track will
# make install.sh (step 6) see a newer-and-different file, refuse to clobber it,
# and leave the box running the generated config instead of ours — silently, as
# a WARN in a wall of output. Move known-generated files aside so the symlink
# can land. Only touches a REGULAR file that still carries the generator's
# fingerprint; a symlink or a file you have actually edited is left alone.
stash_generated() {   # stash_generated <path> <fingerprint-regex>
  local f="$1" sig="$2"
  [[ -e "$f" && ! -L "$f" ]] || return 0
  grep -qE "$sig" "$f" 2>/dev/null || return 0
  mv "$f" "$f.generated-$(date +%Y%m%d%H%M%S)"
  log "  moved generated $(basename "$f") aside so install.sh can link ours"
}
stash_generated "$HOME/.zshrc"    'ZSH_THEME="robbyrussell"|export ZSH="\$HOME/\.oh-my-zsh"'
stash_generated "$HOME/.p10k.zsh" 'Generated by Powerlevel10k configuration wizard'
